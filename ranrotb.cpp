...
// then define it thus:
uint32_t _lrotl (uint32_t x, int r) {
...
double TRanrotBGenerator::Random() {
  ...
  if (randbuffer[p1] == randbufcopy[0] &&
    memcmp(randbuffer, randbufcopy+KK-p1, KK*sizeof(uint32_t)) == 0) {
      // self-test failed
      if ((p2 + KK - p1) % KK != JJ) {
...
void TRanrotBGenerator::RandomInit (long int seed) {
  ...
  memcpy (randbufcopy, randbuffer, KK*sizeof(uint32_t));
  memcpy (randbufcopy+KK, randbuffer, KK*sizeof(uint32_t));
#endif
  // randomize some more
